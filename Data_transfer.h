...
public:
    ...
    void program_counter_function()
    {
        ...
        {
            //  x=fileptr.tellg();
            reobj.program_counter_next.insert({reobj.PC, line}); //.insert({key,value})..
            getline(fileptr, str);
            ...
            if (reobj.command_address.find(word) == reobj.command_address.end())
            {
                if (word[word.length() - 1] == ':')
                {
                    ...
                    reobj.labelJump.insert({temp, line});
                    stream >> word;
                    if (reobj.command_address.find(word) == reobj.command_address.end())
                    {
                        print8085ErrorPC(line);
                    }
                    else
                    {
                        reobj.PC += reobj.command_address[word];
                    }
                }
                else
                {
    ...
    void put_data()
    {
        int ch;
        string add, x;
        cout << "The starting address for taking data starts from 2000 and ends at 2999 and for storing output at 8000 and ends at 8050 .\n";
        cout << "Do not include H in address when entered and spaces after the instruction .\n";
        while (1)
        {
            cout << "0 to exit and 1 to continue ." << endl;
            cin >> ch;
            if (ch == 0)
                break;
            cout << "Enter the address and its corresponding value :\n";
            cin >> add;
            cin >> x;
            address_obj.arr_address[add] = x;
        }
    }
    ...
    void HLT()
    {
        int ch;
        string add;
        cout << "flag register carry"
             << " " << reobj.flag_register["F"] << endl;
        cout << "flag register auxillary carry"
             << " " << reobj.flag_register["AC"] << endl;
        cout << "flag register sign"
             << " " << reobj.flag_register["S"] << endl;
        cout << "flag register parity"
             << " " << reobj.flag_register["P"] << endl;
        cout << "flag register zero"
             << " " << reobj.flag_register["Z"] << endl;
        while (1)
        {
            cout << "0 to exit and 1 to continue ." << endl;
            cin >> ch;
            if (ch == 0)
                break;
            cout << "Enter the address at which you want to see the output" << endl;
            cin >> add;
            cout << address_obj.arr_address[add] << endl;
        }
    }
    int gv = 0;
